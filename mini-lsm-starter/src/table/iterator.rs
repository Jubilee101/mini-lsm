...
impl SsTableIterator {
    ...
    pub fn create_and_seek_to_key(table: Arc<SsTable>, key: KeySlice) -> Result<Self> {
        ...
        if !block_iter.is_valid() {
            // current block has no key larger than target key
            if blk_idx + 1 < table.num_of_blocks() {
                blk_idx += 1;
                block_iter =
                    BlockIterator::create_and_seek_to_key(table.read_block_cached(blk_idx)?, key);
            }
    ...
    pub fn seek_to_key(&mut self, key: KeySlice) -> Result<()> {
        ...
        if !block_iter.is_valid() {
            // current block has no key larger than target key
            // if we can't advance anymore just stay as invalid
            if blk_idx + 1 < self.table.num_of_blocks() {
                blk_idx += 1;
                block_iter = BlockIterator::create_and_seek_to_key(
